...
describe("Gate 2 — the only dependency delta is the v8-sanctioned pdfkit pair (milestone invariant, retargeted plan 25-01)", () => {
  ...
  // types. The empty-diff assertion this gate made is no longer true, so it is
  // retargeted IN PLACE (not deleted, not loosened): the delta vs this phase's
  // base must be EXACTLY `+pdfkit` in `dependencies` and `+@types/pdfkit` in
  // `devDependencies`, every shared range byte-identical, nothing removed —
  // parsed from JSON, never regexed. A second added package fails this BY NAME.
  it("adds exactly pdfkit + @types/pdfkit vs the phase base and changes no other package.json key", () => {
    const base = JSON.parse(
      execSync(`git show ${PHASE_20_BASE}:package.json`, {
        encoding: "utf8",
        cwd: process.cwd(),
      }),
    );
